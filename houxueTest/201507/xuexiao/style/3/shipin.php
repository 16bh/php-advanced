...
else
{
	...
	if(!empty($idlist))
	{
		foreach($idlist as $value)
		{
			...
			{
				//获取预览图
				$sSql = "select videostore_id from T_Video where Id=".intval($row["VideoId"])." limit 1";
				$rtn = $dbh2->query($sSql)->fetch(PDO::FETCH_ASSOC);
				$param = array("id" => $rtn["videostore_id"], "width" => "60", "height" => "60");
				$jsondata = VideoStore::getVideoPreviewImageUrl($param);
				$src = $jsondata["data"];
				$row["imagepreviewurl"] = $src;
			}
