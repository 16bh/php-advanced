...
if($flag=="detail")
{
	...
	$sSql="select * from T_StudyDocs where Id='".$id."' and LoginId='".$school["LoginId"]."'";
	...
	if($row["DocFormat"]=="doc" || $row["DocFormat"]=="docx")
	{
		$row["DocFormatClass"]='word';
	}
	elseif($row["DocFormat"]=="pdf")
	{
		$row["DocFormatClass"]='pdf';
	}
	elseif($row["DocFormat"]=="xls"||$row["DocFormat"]=="xlsx")
	{
		$row["DocFormatClass"]='xls';
	}
	elseif($row["DocFormat"]=="ppt"||$row["DocFormat"]=="pptx")
	{
		$row["DocFormatClass"]='ppt';
	}
	elseif($row["DocFormat"]=="txt")
	{
		$row["DocFormatClass"]='a';
	}
	$row["FileSize"]=$row["FileSize"]<1 ? 1 : $row["FileSize"];
	$row["FileSizeDisplay"]=changeFileSize($row["FileSize"]);

	$row["Visit"]=$row["Visit"]<=0 ? 0 : $row["Visit"];
	$row["DownloadCount"]=$row["DownloadCount"]<=0 ? 0 : $row["DownloadCount"];

	$link=$_SERVER['REMOTE_ADDR'].date('YmdH')."http://www.houxue.com/ziliao/".$row["Id"].".htm";
	$row['hash'] = substr(md5($link), 0, 24);

	$smarty->assign("row", $row);

	//浏览数+1
	$sSql="update T_StudyDocs set Visit=".$row["Visit"]."+1 where Id='".$row["Id"]."'";
	$rtn = $dbh2->exec($sSql);if($rtn === false)die(date("\n\n Y-m-d H:i:s\t").__file__." line ".__line__."\t".$sSql);

	$tplFile = "ziliao_detail.htm"; //使用框架结构式模板
}
else
{
	...
	//取得列表
	$sSql  = " select SQL_CALC_FOUND_ROWS Id from T_StudyDocs where 1=1 and LoginId='".$school["LoginId"]."' order by Id DESC $slimit";
	...
	if(!empty($idlist))
	{
		...
		{
			$sSql="select * from T_StudyDocs where Id=".$value["Id"];
			$row = $dbh2->query($sSql)->fetch(PDO::FETCH_ASSOC);
...
{
	$tpl_output =preg_replace('/ziliao\.php\?id=([0-9]+)*&pager_rownum=([0-9]+)*/','\1/ziliao/list-\2.htm', $tpl_output);
	return $tpl_output;
}
